...
public class Indicator3D {
    ...
    private static void onWorldRender(WorldRenderContext context) {
        ...
        // ---------------------------------------------------------
        // 1. SOUL RAGDOLL RENDERER (Ascension Animation) - CRASH FIXED!
        // ---------------------------------------------------------
        if (ModConfig.soulAnimationEnabled) {
            ...
            while (iter.hasNext()) {
                ...
                // FIX: Used specific drawSoulQuad to provide UV1 (Overlay) and Normal!
        ...
        for (Entity entity : client.world.getEntities()) {
            ...
            if (lastHealthMap.containsKey(id)) {
                ...
                if (lastHealth > 0 && health <= 0) {
                    if (target.distanceTo(client.player) < 15.0) { 
            ...
            if (health <= 0 || target.isInvisible() || target.distanceTo(client.player) > 64.0) {
                continue;
            }
                ...
                if (ModConfig.dragonAuraEnabled) {
                    ...
                    if (alpha > 0.05f) {
                        ...
                        if (drawHead && shatter == 0) {
                            draw3DBox(mat, quadBuffer, headX, headY+0.1f, headZ, 0.2f, 1f, 0f, 0f, alpha, 15728880);
                        }
                    }
                    matrices.pop();
                }
    ...
    // ---------------------------------------------------------
    // --- EXPANDED HELPER RENDERING METHODS ---
    // ---------------------------------------------------------
    ...
    // --- FIX FOR CRASH: SOUL ANIMATION REQUIRES OVERLAY & NORMAL ---
