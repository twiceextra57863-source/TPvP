...
public class Indicator3D {
    ...
    private static void onWorldRender(WorldRenderContext context) {
        ...
        // ---------------------------------------------------------
        // 1. SOUL RAGDOLL RENDERER (CULLING BUG FIXED!)
        // ---------------------------------------------------------
        if (ModConfig.soulAnimationEnabled) {
            ...
            while (iter.hasNext()) {
                ...
                float alpha = 1.0f - (float)Math.pow(life, 2); 
                double waveX = Math.sin(life * 10) * 0.3, waveZ = Math.cos(life * 12) * 0.3, upY = life * 4.0;
                ...
                // FIX: Positive scale so faces aren't culled, then rotate 180 to face camera correctly!
                ...
                drawSoulQuad(mat, soulBuffer, -1, -2, 2, 2, 8f/64f, 8f/64f, 16f/64f, 16f/64f, 1f, 1f, 1f, alpha, l); 
                drawSoulQuad(mat, soulBuffer, -1, 0, 2, 3, 20f/64f, 20f/64f, 28f/64f, 32f/64f, 1f, 1f, 1f, alpha, l); 
                drawSoulQuad(mat, soulBuffer, -2, 0, 1, 3, 44f/64f, 20f/64f, 48f/64f, 32f/64f, 1f, 1f, 1f, alpha, l); 
                drawSoulQuad(mat, soulBuffer, 1, 0, 1, 3, 44f/64f, 20f/64f, 48f/64f, 32f/64f, 1f, 1f, 1f, alpha, l); 
                matrices.pop();
            }
        }
        ...
        for (Entity entity : client.world.getEntities()) {
            ...
            if (health <= 0 || target.isDead()) {
                if (!deadPlayers.contains(id)) {
                    deadPlayers.add(id);
                    if (target.distanceTo(client.player) < 32.0) KillBannerHud.addKill(); 
                ...
                continue; // Don't render indicators for dead bodies
            } else {
                deadPlayers.remove(id); // Reset if they respawn
            }

            if (target.isInvisible() || target.distanceTo(client.player) > 64.0) continue;

            Vec3d tPos = target.getLerpedPos(tickDelta);
            double x = tPos.x - camPos.x, y = tPos.y - camPos.y, z = tPos.z - camPos.z;

            // ---------------------------------------------------------
            // --- DRAGON AURA (NORMAL & TOTEM POP ANIMATIONS) ---
            // ---------------------------------------------------------
            if (target.getName().getString().equals(activeTarget)) {
                ...
                if (ModConfig.dragonAuraEnabled) {
                    ...
                    // --- TOTEM POP EPIC ANIMATION MODIFIERS ---
                    ...
                    if (isTotemPop) {
                        ...
                        if (popT < 0.2f) { // Phase 1: SHOOT UP
                            yOff = (popT / 0.2f) * 6.0f;
                        } else if (popT < 0.4f) { // Phase 2: DIVE DOWN
                            yOff = 6.0f - (((popT - 0.2f) / 0.2f) * 6.0f);
                        } else if (popT < 0.7f) { // Phase 3: SUPER TWIRL
                            rotMult = 10.0f; // Spin like crazy
                        } else { // Phase 4: SHATTER
                            shatter = (popT - 0.7f) / 0.3f;
                        ...
                        if (tNorm > 0.6f) {
                            ...
                            if (shatter > 1.0f) { shatter = 1.0f; alpha = 0f; } else { alpha = 0.8f * (1.0f - shatter); }
                        }
                    }
                    ...
                    if (alpha > 0.05f) {
                        ...
                        for (int layer = 0; layer < 3; layer++) {
                            ...
                            for (int i = 0; i < segments; i++) {
                                ...
                                if (i > segments - 5) { green = 0.8f; blue = 0f; } // Gold tip
                                if (isTotemPop) { green = 0.8f; blue = 0.2f; } // Make it totally Golden during Totem pop!
    ...
    private static void drawSoulQuad(Matrix4f m, VertexConsumer v, float x, float y, float w, float h, float u1, float v1, float u2, float v2, float r, float g, float b, float a, int l) { v.vertex(m, x, y, 0).color(r, g, b, a).texture(u1, v1).overlay(net.minecraft.client.render.OverlayTexture.DEFAULT_UV).light(l).normal(0, 0, 1); v.vertex(m, x, y+h, 0).color(r, g, b, a).texture(u1, v2).overlay(net.minecraft.client.render.OverlayTexture.DEFAULT_UV).light(l).normal(0, 0, 1); v.vertex(m, x+w, y+h, 0).color(r, g, b, a).texture(u2, v2).overlay(net.minecraft.client.render.OverlayTexture.DEFAULT_UV).light(l).normal(0, 0, 1); v.vertex(m, x+w, y, 0).color(r, g, b, a).texture(u2, v1).overlay(net.minecraft.client.render.OverlayTexture.DEFAULT_UV).light(l).normal(0, 0, 1); }
}
