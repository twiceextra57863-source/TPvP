...
public class Indicator3D {
    ...
    // KILL TRACKING MAP & CRASH-SAFE LIST
    private static final Map<Integer, Float> lastHealthMap = new HashMap<>();
    private static final List<DeadSoul> activeSouls = new CopyOnWriteArrayList<>(); // Concurrent Crash Fix!
    ...
    private static void onWorldRender(WorldRenderContext context) {
        ...
        // 1. RENDER RAGDOLL SOULS
        if (ModConfig.soulAnimationEnabled) {
            long now = System.currentTimeMillis();
            for (DeadSoul soul : activeSouls) {
                long age = now - soul.startTime;
                if (age > 4000) { activeSouls.remove(soul); continue; } 
                ...
                drawTextureQuad(mat, soulBuffer, -1, -2, 2, 2, 8f/64f, 8f/64f, 16f/64f, 16f/64f, 1f, 1f, 1f, alpha, l); 
                drawTextureQuad(mat, soulBuffer, -1, 0, 2, 3, 20f/64f, 20f/64f, 28f/64f, 32f/64f, 1f, 1f, 1f, alpha, l); 
                drawTextureQuad(mat, soulBuffer, -2, 0, 1, 3, 44f/64f, 20f/64f, 48f/64f, 32f/64f, 1f, 1f, 1f, alpha, l); 
                drawTextureQuad(mat, soulBuffer, 1, 0, 1, 3, 44f/64f, 20f/64f, 48f/64f, 32f/64f, 1f, 1f, 1f, alpha, l); 
                matrices.pop();
            }
        }
        ...
        for (Entity entity : client.world.getEntities()) {
            ...
            // SAFE KILL DETECTION (CRASH FIXED)
            if (lastHealthMap.containsKey(id)) {
                float lastHealth = lastHealthMap.get(id);
                if (lastHealth > 0 && health <= 0) { // If died right now
                    try {
                        if (target.distanceTo(client.player) < 15.0) KillBannerHud.addKill(); 
                        if (target instanceof AbstractClientPlayerEntity pt) {
                            // Safe Skin Texture Check to prevent crash
                    ...
                    } catch (Exception e) { /* Ignore crash safely */ }
                }
            }
            ...
            // --- 3. CINEMATIC DRAGON AURA (Rise -> Hold -> Shatter -> Rest) ---
            if (target.getName().getString().equals(activeTarget)) {
                if (ModConfig.dragonAuraEnabled) {
                    ...
                    // Cinematic Timing Logic
                    long cycle = 4000; // 4 second loop
                    float t = (System.currentTimeMillis() % cycle) / (float) cycle;
                    
                    float progress = 0f, shatter = 0f, alpha = 0.8f;
                    if (t < 0.4f) { progress = t / 0.4f; } // Phase 1: Rise
                    else if (t < 0.5f) { progress = 1.0f; } // Phase 2: Hold
                    else if (t < 0.6f) { // Phase 3: Blast/Shatter
                        progress = 1.0f; 
                        shatter = (t - 0.5f) / 0.1f; // 0 to 1 explosion
                        alpha = 0.8f * (1.0f - shatter); 
                    } else { alpha = 0f; } // Phase 4: Rest
                    ...
                    if (alpha > 0.05f) {
                        ...
                        for (int layer = 0; layer < 3; layer++) {
                            ...
                            for (int i = 0; i < activeSegments; i++) {
                                ...
                                float rot = (System.currentTimeMillis() % 2000) / 2000.0f;

                                float py1 = pt1 * h;
                                float px1 = (float) Math.cos((pt1 + offset + rot) * Math.PI * 4) * radius;
                                float pz1 = (float) Math.sin((pt1 + offset + rot) * Math.PI * 4) * radius;

                                float py2 = pt2 * h;
                                float px2 = (float) Math.cos((pt2 + offset + rot) * Math.PI * 4) * radius;
                                float pz2 = (float) Math.sin((pt2 + offset + rot) * Math.PI * 4) * radius;

                                // DUST / SHATTER EFFECT (Particles scatter outward)
                                if (shatter > 0) {
                                    float sx = (float) Math.sin(i * 13 + layer) * shatter * 2.0f;
                                    float sy = (float) Math.cos(i * 17 + layer) * shatter * 2.0f;
                                    float sz = (float) Math.sin(i * 23 + layer) * shatter * 2.0f;
                                    px1 += sx; py1 += sy; pz1 += sz;
                                    px2 += sx; py2 += sy; pz2 += sz;
                                }

                                if (layer == 1 && i == activeSegments - 1) { headX = px2; headY = py2; headZ = pz2; drawHead = true; }

                                float red = 1f, green = 0.1f, blue = 0.1f;
                                if (i > segments - 5) { green = 0.8f; blue = 0f; } // Gold at top

                                drawQuad(mat, quadBuffer, px1, py1, pz1, px1, py1+0.15f, pz1, px2, py2+0.15f, pz2, px2, py2, pz2, red, green, blue, alpha, 15728880);
                            }
                        }
                        if (drawHead && shatter == 0) draw3DBox(mat, quadBuffer, headX, headY+0.1f, headZ, 0.2f, 1f, 0f, 0f, alpha, 15728880);
                    }
                    matrices.pop();
                }
                ...
                client.textRenderer.draw("▼", -client.textRenderer.getWidth("▼")/2f, 0, 0xFFFF2222, true, matrices.peek().getPositionMatrix(), immediate, TextRenderer.TextLayerType.SEE_THROUGH, 0, 15728880);
                matrices.pop();
            }
