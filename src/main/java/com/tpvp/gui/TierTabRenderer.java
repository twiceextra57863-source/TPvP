...
public class TierTabRenderer {
    ...
    public static int viewState = 0; // 0 = Circle, 1 = Modes, 2 = Stats
    ...
    public static void render(TPvPDashboardScreen screen, DrawContext context, int setX, int setY, int mx, int my, int winW, int winH) {
        ...
        animProgress = 1.0f - (float)Math.pow(1.0f - animProgress, 3); // Smooth slow-down
        ...
        // ---------------------------------------------------------
        // STATE 0: ANIMATED REVERSE CIRCLE
        // ---------------------------------------------------------
        if (viewState == 0) {
            float skillPct = PvPStatsManager.calculateSkillPercentage("Overall");
            
            // FIX: Starts at 100%, shrinks down to actual skill percentage!
            float currentShowPct = 100f - ((100f - skillPct) * animProgress); 
            if (skillPct == 0 && animProgress > 0.99f) currentShowPct = 0f; // Snap to 0 if unranked
            ...
            // Foreground Circle (Dynamic Colors)
        ...
        // ---------------------------------------------------------
            ...
            if (PvPStatsManager.matchHistory.isEmpty()) {
                ...
                for (int i = 0; i < Math.min(5, PvPStatsManager.matchHistory.size()); i++) {
    ...
    private static void drawModeCard(TPvPDashboardScreen screen, DrawContext context, int x, int y, int mx, int my, String name, ItemStack icon) {
        ...
        context.getMatrices().push(); context.getMatrices().translate(x + 10, y + 10, 0); context.getMatrices().scale(2.0f, 2.0f, 1.0f);
        context.drawItem(icon, 0, 0); context.getMatrices().pop();
    ...
    public static boolean mouseClicked(double mx, double my, int setX, int setY) {
        if (viewState == 0) {
            if (mx >= setX + 110 && mx <= setX + 210 && my >= setY + 150 && my <= setY + 175) { viewState = 1; openTime = System.currentTimeMillis(); return true; }
        } else if (viewState == 1) {
            if (mx >= setX && mx <= setX + 80 && my >= setY + 150 && my <= setY + 170) { viewState = 0; openTime = System.currentTimeMillis(); return true; }
    ...
    private static boolean checkCardClick(double mx, double my, int x, int y, String mode) {
        if (mx >= x && mx <= x + 140 && my >= y && my <= y + 50) {
            selectedMode = mode; viewState = 2; openTime = System.currentTimeMillis(); return true;
        }
        return false;
    }
}
