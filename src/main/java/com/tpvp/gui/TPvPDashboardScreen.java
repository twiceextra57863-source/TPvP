...
public class TPvPDashboardScreen extends Screen {
    private final Screen parent;
    private String currentTab = "Combat";
    private boolean showingFriends = false; // NAYA: Toggle for Friends/Enemies List
    private int scrollOffset = 0;
    private boolean isDraggingScroll = false;

    private final int winW = 460, winH = 260, sideW = 120;

    public TPvPDashboardScreen(Screen parent) { super(Text.literal("TPvP Dashboard")); this.parent = parent; }
    ...
    private void drawTab(DrawContext context, String name, int x, int y, int mx, int my) {
        ...
        if (sel) { context.fill(x, y, x + sideW, y + 20, 0x55FF0000); context.fill(x, y, x + 3, y + 20, 0xFFFF0000); } 
        else if (hov) context.fill(x, y, x + sideW, y + 20, 0x22FFFFFF);
    ...
    @Override public boolean mouseReleased(double mx, double my, int button) { isDraggingScroll = false; return super.mouseReleased(mx, my, button); }
    @Override public boolean mouseScrolled(double mx, double my, double h, double scroll) { if (currentTab.equals("Targets")) { scrollOffset = Math.max(0, scrollOffset - (int) scroll); return true; } return super.mouseScrolled(mx, my, h, scroll); }
